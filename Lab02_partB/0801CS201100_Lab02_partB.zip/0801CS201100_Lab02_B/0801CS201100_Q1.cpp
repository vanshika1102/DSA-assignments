...
public:
 ...
 void addProcess(Process *p){ // user can give a new process to CPU Handler for cpu needs
  ...
  // logic 1: taking shortest remaining time cpu process first
  Process * getMin ()
  {
    Process *p =procList.front();
    if(p!=NULL)
    {
      int curTime=p->currentSlotRemainingTime;
      std::list<Process *>::iterator it,temp=procList.begin();
      for (it = procList.begin(); it != procList.end(); ++it) 
      {
        if((*it)->currentSlotRemainingTime<curTime && (*it)->currentSlotRemainingTime>0){
          p= *it;
          temp=it;
          curTime=(*it)->currentSlotRemainingTime;
        }
      }
      //adding shortest process in front
      procList.erase(temp);
